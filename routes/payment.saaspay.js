 ...
 * │    → Corps webhook SaaSPay (payment.completed)              │
 * │    → Validation signature HMAC X-SaaSPay-Signature         │
...
const controller = require('../controllers/saaspayController');
...
function webhookLogger(req, res, next) {
  logger.info('[SaaSPay Route] Webhook reçu', {
    ip         : req.ip,
    method     : req.method,
    event      : req.headers['x-saaspay-event'] || 'unknown',
    delivery   : req.headers['x-saaspay-delivery'] || 'unknown',
...
// ══════════════════════════════════════════════════════════════
//  ROUTE 2 : POST /api/payment/webhook/saaspay
