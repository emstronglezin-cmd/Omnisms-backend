...
async function activatePremiumFirestore(userId, { checkoutId, transactionId, amount, currency, paymentMethod, paidAt }) {
  ...
  try {
    ...
    logger.info('[SaaSPay] ✅ Premium activé', { userId, checkoutId, transactionId });
  } catch (err) {
    logger.error('[SaaSPay] Erreur activation premium Firestore', { userId, checkoutId, error: err.message });
  }
}
...
async function createPayment(req, res) {
  ...
  try {
    ...
    logger.error('[SaaSPay] Erreur création checkout', { userId: cleanUserId, orderId, error: err.message });
  ...
  logger.info('[SaaSPay] Checkout créé ✅', {
    userId    : cleanUserId,
    orderId,
    checkoutId: checkout.checkoutId,
...
async function handleWebhook(req, res) {
  ...
  const signature = req.headers['x-saaspay-signature'] || '';
...
async function processWebhookPayload(body, rawBody, signature, event) {
  ...
  if (!saaspay.verifyWebhookSignature(rawBody, signature)) {
...
async function handleSuccessfulPayment({ checkoutId, transactionId, userId, orderId, amount, currency, paymentMethod, paidAt, customer }) {
  logger.info('[SaaSPay] Paiement confirmé (paid) ✅', { checkoutId, transactionId, userId, amount, currency });

  // Anti-concurrent
  if (checkoutId && processingPayments.has(checkoutId)) {
    logger.info('[SaaSPay] Traitement concurrent — ignoré', { checkoutId });
    return;
  }

  // Anti-replay
  if (checkoutId && await isAlreadyProcessed(checkoutId)) {
    logger.info('[SaaSPay] Déjà traité (idempotence)', { checkoutId });
    return;
  }
  ...
  try {
    ...
    if (!resolvedUserId && checkoutId) {
      try {
        ...
        if (ckSnap.exists && ckSnap.data()?.userId) {
          ...
          logger.info('[SaaSPay] userId récupéré via Firestore checkoutId', {
        ...
        if (!resolvedUserId && orderId) {
          ...
          if (orSnap.exists && orSnap.data()?.userId) {
            ...
            logger.info('[SaaSPay] userId récupéré via Firestore orderId', {
        ...
        if (!resolvedUserId) {
          ...
          if (!querySnap.empty && querySnap.docs[0].data()?.userId) {
            ...
            logger.info('[SaaSPay] userId récupéré via Firestore query checkoutId', {
              checkoutId, resolvedUserId,
            });
          }
        }
      } catch (lookupErr) {
        logger.warn('[SaaSPay] Erreur lookup userId Firestore', { error: lookupErr.message, checkoutId });
      }
    }

    if (!resolvedUserId) {
      logger.error('[SaaSPay] ⚠️ CRITIQUE : userId introuvable — activation premium IMPOSSIBLE', {
        checkoutId,
        orderId,
    ...
    logger.info('[SaaSPay] Activation terminée ✅', { userId, checkoutId });
...
async function getPaymentStatus(req, res) {
  ...
  try {
    const statusData = await saaspay.getCheckoutStatus(cleanId);
    return res.status(200).json({
      success         : true,
      source          : 'leekpay_api',
